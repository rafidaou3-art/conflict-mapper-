...
# New google-genai SDK: create a client (the old google-generativeai package is deprecated).
client = genai.Client(api_key=api_key)

MODEL_NAME = "gemini-2.5-flash"

# The key fix: thinking_budget=0 turns OFF the model's internal "thinking", which on
# gemini-2.5-flash is ON by default and silently eats your output-token budget --
# that is what was producing empty / truncated responses and the JSON errors.
# response_mime_type forces clean JSON (no ``` fences), and the high token cap
# prevents long timeline answers from being cut off mid-JSON.
def make_config():
...
def read_response_text(response) -> str:
    """Safely pull text out of a google-genai response.

    Accessing response.text can raise or return None when the model returns no
    usable content (blocked, empty, or MAX_TOKENS). This surfaces a clear reason
    instead of a cryptic JSON parse error downstream.
    """
    text = None
    try:
        text = response.text
    ...
    # Dig into candidates to explain *why* it was empty.
    reason = "unknown"
    try:
    ...
    if "MAX_TOKENS" in reason:
        raise ValueError(
            ...
            "Try shorter input text, or raise max_output_tokens."
        )
    if "SAFETY" in reason or "BLOCK" in reason:
        raise ValueError("The model blocked this response (safety filter). Try rephrasing the text.")
    raise ValueError(f"The model returned an empty response (finish reason: {reason}).")
...
def extract_json_object(text: str):
    ...
    # JSON mode usually removes these, but keep the guard as a safety net.
    ...
    raise ValueError(f"Model did not return valid JSON. Raw response: {cleaned[:1200]}")
...
def analyze():
    try:
        ...
        if include_timeline and not result.get("timeline"):
            return json_error(
                "Response was parsed, but no timeline data was found.",
                500,
        ...
        if not include_timeline and not result.get("actors"):
            return json_error(
                "Response was parsed, but no actor data was found.",
                500,
                raw_response=response_text[:1200],
            )
